<template>
  <div class="p-6 relative">
    <div class="relative flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ t('categories.editCategory') }}</h1>
      <NuxtLink
        to="/categories"
    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6 mx-auto max-w-screen-lg">
      <form @submit.prevent="handleUpdateCategory" class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- Category Name -->
        <div>
        ...
        <div class="md:col-span-2 text-center">
          <button type="submit" :disabled="isLoading" class="submit-button">
            {{ isLoading ? t('categories.updating') : t('categories.updateCategory') }}
          </button>
        </div>
      </form>
...
const { fetchCategoryById, updateCategory, selectedCategory } = useCategory();
...
const categoryId = route.params.id; // Get the category ID from route params

// Fetch category data by ID when the page loads
onMounted(() => {
  fetchCategoryById(route.params.id)
})
...
const handleUpdateCategory = async () => {
  ...
  const result = await updateCategory(categoryId, formData.value);
  if (result.success) {
    notificationStore.showNotification(t('categories.updateSuccess'), 'success');
    router.push('/categories');
  } else {
    notificationStore.showNotification(result.message || t('categories.updateFail'), 'error');
  }
